...
# Add the agent module to the path to import the gemini-agent
from .gemini_agent.graph import graph as gemini_search_agent
...
class GeminiResearchAgent:
    ...
    async def research(self, query: str, research_report: str = "research_report",
        ...
        """Conduct research using the gemini-search-agent instead of GPTResearcher."""
        
        if verbose:
            if self.websocket and self.stream_output:
                await self.stream_output("logs", "research", f"Starting Gemini research on: {query}", self.websocket)
            else:
                print_agent_output(f"Starting Gemini research on: {query}", agent="GEMINI_RESEARCHER")

        # Prepare the state for the gemini-search-agent
        initial_state = {
        ...
        # Configuration for the gemini-search-agent
        config = {
        ...
        try:
            # Run the gemini-search-agent
            result = await gemini_search_agent.ainvoke(initial_state, config)
            ...
            if verbose:
                if self.websocket and self.stream_output:
                    await self.stream_output("logs", "research", f"Gemini research completed for: {query}", self.websocket)
                else:
            ...
            error_msg = f"Error in Gemini research for query '{query}': {str(e)}"
            if verbose:
                if self.websocket and self.stream_output:
    ...
    async def run_subtopic_research(self, parent_query: str, subtopic: str, verbose: bool = True, source="web", headers=None):
        """Run research on a subtopic using the gemini-search-agent."""
        try:
    ...
    async def run_initial_research(self, research_state: dict):
        """Run initial research using the gemini-search-agent."""
    ...
    async def run_depth_research(self, draft_state: dict):
        """Run in-depth research on a specific topic using the gemini-search-agent."""
