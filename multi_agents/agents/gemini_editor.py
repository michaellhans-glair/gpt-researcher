...
class GeminiEditorAgent:
    ...
    async def plan_research(self, research_state: Dict[str, any]) -> Dict[str, any]:
        ...
        print_agent_output(
            "Planning an outline layout based on initial Gemini research...", agent="GEMINI_EDITOR")
    ...
    def _log_parallel_research(self, queries: List[str]) -> None:
        """Log the start of parallel Gemini research tasks."""
        if self.websocket and self.stream_output:
            ...
            print_agent_output(
                f"Running the following Gemini research tasks in parallel: {queries}...",
                agent="GEMINI_EDITOR",
            )

    def _create_task_input(self, research_state: Dict[str, any], query: str, title: str) -> Dict[str, any]:
        """Create the input for a single Gemini research task."""
        return {
            "task": research_state.get("task"),
            "topic": query,
            "title": title,
